...
class VertexMover(tk.Toplevel):
    ...
    def __create_spinbox(self, frame, column, text, command):
        ...
        spinbox = ttk.Spinbox(frame, textvariable=var, from_=-100 * self.N, to=100 * self.N, state="disabled",
                              command=command, validate='all', validatecommand=c_vcmd)
...
class RotationInterface(ttk.Frame):
    ...
    def __create_slider(self, text, variable):
        ...
        ttk.Scale(slider_frame, variable=variable, from_=-PI, to=PI, command=self.graph.draw).grid(row=0, column=1,
                                                                                                   sticky='ew', padx=2)
...
class Controls(ttk.Frame):
    ...
    def __create_zoom_slider(self, variable):
        ...
        ttk.Scale(self, variable=variable, from_=400.0, to=10, orient="horizontal", command=self.graph.draw).pack(
            pady=3, padx=5, fill='x')
...
class GUI(tk.Tk):
    ...
    def __create_import_button(self):
        ttk.Button(self, text='Import graph', command=self.import_graph).place(relx=0.905, rely=0.004, relwidth=0.1,
